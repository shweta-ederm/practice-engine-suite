...
- Data viz: `.bar-chart` (+`.bar.emph`), `.bar-pair` (grouped two-series bars per category), `.donut` (conic gradient, `--donut-value`), `.progress`, `.legend`.
- Report widgets (Analytics pages only, so Analytics never looks like a second dashboard): `.report-hero` (ink band; eyebrow, accent-colored headline value, caption, supporting `.hero-stat`s), `.trend-chart` (styles a single-series inline SVG line: `.trend-grid / -area / -line / -dot / -tag`, latest point emphasized), `.heatmap` (sequential single-hue cells `.hm-0` to `.hm-5` built from the accent, row/col labels, always with the `.hm-legend` ramp), `.ring` + `.ring-block` (goal ring via `--val` 0-100), `.bullet-track / -fill / -target` (hbar layout plus a target tick via `--t`; note when shorter is better), `.pipe` (numbered `.pipe-step`s with `.pipe-drop` hand-off rates between stages), `.chip-delta` (`.up / .down / .flat` year-over-year movement in tables).
...
| Recall Health: Analytics | recall-health-analytics.html |
| Medical Receptionist | ai-receptionist.html |
| Medical Receptionist: Analytics | ai-receptionist-analytics.html |
| Chatbot | ai-chatbot.html |
| Chatbot: Analytics | ai-chatbot-analytics.html |
| Online Scheduler | appointment-scheduler.html |
| Online Scheduler: Analytics | appointment-scheduler-analytics.html |
| Patient Intake | patient-intake.html |
| Patient Intake: Analytics | patient-intake-analytics.html |
| Pay My Doc | pay-my-doc.html |
| Pay My Doc: Analytics | pay-my-doc-analytics.html |
...
| PE Admin: Billing and Plans | pe-admin-billing.html |
| PE Admin: Integrations | pe-admin-integrations.html |
| PE Admin: System Health | pe-admin-health.html |
| PE Admin: Platform Settings | pe-admin-settings.html |
...
- 2026-07-28 (v8.3): Fixed Recall Health's invisible Revenue by Provider chart: its grouped bars used a `.bar-pair` wrapper that had no CSS, so the pairs collapsed to zero size and the dashboard lost its chart colors with them; `.bar-pair` is now a documented component. Patient Intake dashboard dropped the "Patient view: new patient form set" preview card; Intake queue and Recently added forms sit side by side. Integrations now name the real dermatology stack (ModMed EMA, Nextech, athenahealth, eClinicalWorks for PMS/EHR bridges; Twilio, SendGrid, Stripe for platform services) and the practice detail PM/EHR fields match. Pay My Doc repositioned as the platform's third-strongest product (760 practices, $89K MRR) across the PE Admin adoption chart, Product Dashboards, Billing revenue chart (platform MRR now $527K), and the deck's control-plane miniature.
- 2026-07-28 (v8.2): Four PE Admin modules built and every admin sidebar wired to them. Billing and Plans (pe-admin-billing.html) models pricing in three layers so it stays robust but simple: a Price Book tab (per-product base subscription plus at most one usage meter with an included allowance: per patient reactivated, per completed intake, per answered minute, per conversation, per payment collected; Online Scheduler stays flat), a Plans and Bundles tab (three standard bundles with savings and practice counts, plus a custom plans table where every deviation from the price book is named, dated, and badged Active/Trial/Grandfathered), an Invoices tab (base + metered usage per line), an Overview tab (MRR and usage KPIs, product-colored revenue chart, subscription-vs-usage mix), and a New Plan or Bundle modal where blank fields inherit the price book. Integrations (pe-admin-integrations.html): PMS/EHR bridge table with sync health, six platform service cards (SMS, voice, email, payments, maps, calendars, all fictional vendors), and a Needs Attention queue. System Health (pe-admin-health.html): uptime/latency KPIs, per-product and shared-service status lists, a 30-day status strip, incident history, and the maintenance window. Platform Settings (pe-admin-settings.html): organization, security policy, new-practice defaults, communications, data and compliance, and API cards.
- 2026-07-27 (v8.1): Analytics label clarity pass: month axes use three-letter abbreviations (Aug through Jul) instead of ambiguous single initials, and time-of-day heatmaps now run six two-hour columns labeled with full am/pm times (8am, 12pm, 6pm) instead of twelve cramped a/p columns; insight copy updated to match. Recall Health's sidebar Analytics is a single link like every other app (the old five-item dropdown collapsed into the one analytics page). PE Admin practice settings: product activation toggles are live; turning the Chatbot on reveals its Widget, Conversation, and Knowledge Base and Lead Capture settings in place of the inactive note, flips the badge, and un-dims its picker tile (elements marked data-when-active / data-when-inactive, wired generically for all products).
- 2026-07-27 (v8.0): Analytics pages rebuilt with their own widget family after feedback that they read like second dashboards. Dashboards keep stat cards, bar charts, and donuts; Analytics pages now open with an ink `.report-hero` (the one headline number in the product color, plus supporting stats), then a single-series SVG trend line with the latest point tagged, a numbered step pipeline with hand-off rates replacing funnel bars, an hour-by-day (or channel-by-month) sequential heatmap with a ramp legend and the "Marketing gold" insight, a goals card of progress rings plus target-tick bullet bars, and a leaderboard table with year-over-year delta chips. All widgets documented in a new Report widgets section of the design system with usage rules (single hue heatmaps from the accent, one series per trend line, target ticks noted when shorter is better). Deck trimmed to eight slides (Flexibility, Scalability, and The Demo removed), the closing slide centered, and the deck itself now renders as a 16:9 presentation stage centered on the ink backdrop like a PowerPoint slide instead of filling the screen.
- 2026-07-24 (v7.9): Every product gained a stakeholder Analytics page behind its sidebar Analytics link (recall-health-analytics.html, ai-receptionist-analytics.html, ai-chatbot-analytics.html, appointment-scheduler-analytics.html, patient-intake-analytics.html, pay-my-doc-analytics.html), all on one template: the global admin filter bar (Period defaulting to Last 12 months, Practice, Locations, Providers), four edge-stat KPIs, a 12-month trend bar chart with the latest month emphasized, a value funnel in horizontal bars closing on a "Marketing gold" insight callout, a you-vs-national (or before/after) benchmark chart with big accent takeaways, and a channel/mix donut with integrated legend or a leaderboard table. Header actions standardized to Schedule Email Report (secondary) and Download Board Report (primary). Every story leads with money or a marketable stat: RH recovered revenue and ROI, Medical Receptionist after-hours bookings saved, Chatbot website-to-patient conversion lift, Scheduler no-show rate versus national, Intake pre-visit completion versus the paper era, PMD days-to-collect and bad-debt reduction. Sidebar Reports items renamed Analytics and wired to these pages on Medical Receptionist, Chatbot, Online Scheduler, and Patient Intake; RH's Analytics sub-items and PMD's Analytics item link to their pages.
- 2026-07-22 (v7.8): deck.html rebuilt as the strategic pre-demo briefing: horizontal presentation with one slide on screen, left/right arrow controls, keyboard navigation, and bottom dot pagination with a counter. Eleven slides cover why (fragmentation vs product family), the three audiences, the One Suite brand voice, the cross-product ecosystem, the stakeholder-first dashboard philosophy, PE Admin as control plane, flexibility (name registry, tokens, theming shown as code chips), scalability, the demo agenda, and a closing call to action. Dashboards appear as faux-browser miniatures rebuilt from live design system components rather than screenshots, so they re-theme with the system and never go stale. A graphics-first pass replaced prose with visuals throughout: a before/after vendor-scatter versus app-mark panel on the Why slide, persona cards reduced to badge chips, icon-point rows (icon chip plus a few words) instead of bullet paragraphs, code chips for the flexibility story, a step-chip pipeline for scalability, and a five-word demo agenda. Print stylesheet still emits one slide per page.
